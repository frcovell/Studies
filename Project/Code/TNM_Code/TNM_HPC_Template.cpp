...
string dir = "/rds/general/user/fc420/home/TNM_Results/";    // Directory for output of model
...
double defImmFrag = 0.008;                     // FC Defult immigration rate
...
void storeParamFrag(string fileName, string outpath);
...
int main(int argc, char *argv[]) {
    ...
    if(argc == 9) {
        ...
        fragName = argv[8];
        fraginpath = "/rds/general/user/fc420/home/Fragments/";
    }
    ...
    if (fragmentation == 1) {
        ...
        storeParamFrag("/FragParameters.txt", fragoutpath);
...
void storeParamFrag(string fileName, string outpath) {
    ...
    file << "Interaction Distance = " << InteractDist << "\n";
    file << "Intraspecific Competition = " << Intraspecific << "\n";
    file << "Immigration Rate  = " << probImmFrag << "\n";
...
double calculateInteractions(double (&J)[numSpec][numSpec], int (&cellPopSpec)[numCells][numSpec], int (&cellPop)[numCells], double (&distArray)[numCells][numCells],
    ...
    validCells = findValidCells(distArray, InteractDist, cell);
