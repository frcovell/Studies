...
AgeAtDeath <- append(x=AgeAtDeath, values =ifelse(Age == Dead,Age,"NA" ), after = (min(Id)-1) )
...
{                           ## print all output of loop
  ...
  AgeAtDeath <- append(x=AgeAtDeath, value = rep( "NA", sum(babiesF)), after = max(which(Gen < max(Gen))))##Error
  
  newDead<- which(Age==Dead & AgeAtDeath == "NA")
  maxAge<- which(Age == max(P.ages)& AgeAtDeath == "NA")
 
  AgeAtDeath[newDead]<-P.ages
  AgeAtDeath[maxAge]<-max(P.ages)
 myPed<- data.frame(Id, Sire, Dam, Sex, Gen, Age, AgeAtDeath)
  
  if (max(Gen) == (nGens +1) ) break; return(myPed); }
#if (max(Gen) == (nGens +1) ) break; test<-myPed; }
  #if (max(Gen) == (nGens +1) ) break; print(myPed<- data.frame(Id, Sire, Dam, Sex, Gen, Age, AgeAtDeath)); }
}


##issues## 
# individuals living longer the max age
# outputs
# while loop outputs all iteration withing the loop not just final ped 
