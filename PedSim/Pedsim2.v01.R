                   ...
                   MatingSyst=NULL)
{
 ...
 if(MatingSyst==2){
    ...
    tempM <- which(Sex=="male" & AgeAtDeath == 0 & Age >= 3 ) 
    tempF <- which(Sex=="female" & AgeAtDeath == 0 & Age >= 3 )
    ...
    #CREATE BREEDING PAIRS
    pairsF <- tempF[breederF]  
    pairsM <- sample(x= which(myKin[breederM] == min(myKin[pairsF])),
